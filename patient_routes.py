...
def patients():
    ...
    if request.method == "POST":
        ...
        patient_transfusions = transfusion.get_transfusions_by_patient(patient_id)
        if patient_transfusions == []:
            search_message = "Ei verensiirtoja"
        else:
            search_message = ""
    return render_template("patients.html", patients=patients, patient_transfusions=patient_transfusions, patient_by_id=patient_by_id, search_message=search_message)
...
def transfusions():
    ...
    if request.method == "POST":
        ...
        if department_transfusions == []:
            ...
            search_message = ""
    return render_template("transfusions.html", products=products, patients=patients, departments=departments, department=department, department_transfusions=department_transfusions, search_message=search_message)
